...
const scanForShaiHulud = (content) => {
  ...
  Object.keys(content.packages).forEach((packageName) => {
    ...
    if (infectedPackages[cleanedName]) {
      console.warn(
        `Infected package found: ${cleanedName} ${infectedPackages[cleanedName]}`
      );
      if (
...
const printStyledReport = (report) => {
  console.log("\n" + "=".repeat(60));
  console.log(
    `${colors.cyan}${colors.bright}🔍 SHAI-HULUD SECURITY SCAN REPORT${colors.reset}`
  );
  console.log("=".repeat(60));

  // Summary section
  console.log(`\n${colors.blue}${colors.bright}📊 SCAN SUMMARY${colors.reset}`);
  console.log(`${colors.dim}${"─".repeat(40)}${colors.reset}`);
  console.log(
    `Total Packages Scanned: ${colors.bright}${report.total}${colors.reset}`
  );
  console.log(
    `Critical Infections: ${
      report.infected.length > 0 ? colors.red + colors.bright : colors.green
    }${report.infected.length}${colors.reset}`
  );
  console.log(
    `Warnings: ${
      report.warning.length > 0 ? colors.yellow + colors.bright : colors.green
    }${report.warning.length}${colors.reset}`
  );

  // Risk level indicator
  if (report.infected.length > 0) {
    console.log(
      `\n${colors.bgRed}${colors.white}${colors.bright} 🚨 CRITICAL RISK DETECTED 🚨 ${colors.reset}`
    );
  } else if (report.warning.length > 0) {
    console.log(
      `\n${colors.bgYellow}${colors.white}${colors.bright} ⚠️  MEDIUM RISK DETECTED ⚠️  ${colors.reset}`
    );
  } else {
    console.log(
      `\n${colors.bgGreen}${colors.white}${colors.bright} ✅ ALL CLEAR - NO THREATS ✅ ${colors.reset}`
    );
  }
  ...
  if (report.infected.length > 0) {
    console.log(
      `\n${colors.red}${colors.bright}🦠 CRITICAL INFECTIONS (${report.infected.length})${colors.reset}`
    );
    console.log(`${colors.red}${"═".repeat(50)}${colors.reset}`);

    report.infected.forEach((pkg, index) => {
      console.log(
        `\n${colors.red}${colors.bright}[${index + 1}] ${pkg.name}${
          colors.reset
        }`
      );
      console.log(
        `    ${colors.dim}├─${colors.reset} Current Version: ${colors.red}${pkg.version}${colors.reset}`
      );
      console.log(
        `    ${colors.dim}└─${colors.reset} Infected Versions: ${colors.red}${pkg.affectedVersions}${colors.reset}`
      );
    });
  }

  // Warning packages section
  if (report.warning.length > 0) {
    console.log(
      `\n${colors.yellow}${colors.bright}⚠️  PACKAGE WARNINGS (${report.warning.length})${colors.reset}`
    );
    console.log(`${colors.yellow}${"═".repeat(50)}${colors.reset}`);

    report.warning.forEach((pkg, index) => {
      console.log(
        `\n${colors.yellow}${colors.bright}[${index + 1}] ${pkg.name}${
          colors.reset
        }`
      );
      console.log(
        `    ${colors.dim}├─${colors.reset} Current Version: ${colors.yellow}${pkg.version}${colors.reset}`
      );
      console.log(
        `    ${colors.dim}└─${colors.reset} Affected Versions: ${colors.yellow}${pkg.affectedVersions}${colors.reset}`
      );
    });
  }
  ...
  console.log(
    `\n${colors.magenta}${colors.bright}💡 SECURITY RECOMMENDATIONS${colors.reset}`
  );
  console.log(`${colors.magenta}${"═".repeat(50)}${colors.reset}`);

  if (report.infected.length > 0) {
    console.log(`${colors.red}🔥 IMMEDIATE ACTIONS REQUIRED:${colors.reset}`);
    console.log(`   • Remove all infected packages immediately`);
    console.log(`   • Scan your system for potential compromise`);
    console.log(`   • Change all API keys and credentials`);
    console.log(`   • Review recent system activity`);
  }

  if (report.warning.length > 0) {
    console.log(`${colors.yellow}⚠️  RECOMMENDED ACTIONS:${colors.reset}`);
    console.log(`   • Update packages to safe versions`);
    console.log(`   • Monitor for security updates`);
    console.log(`   • Consider alternative packages`);
  }

  if (report.infected.length === 0 && report.warning.length === 0) {
    console.log(`${colors.green}✅ SECURITY STATUS: GOOD${colors.reset}`);
    console.log(`   • Continue regular security monitoring`);
    console.log(`   • Keep dependencies updated`);
    console.log(`   • Run periodic scans`);
  }

  console.log(
    `\n${colors.cyan}${
      colors.dim
    }Scan completed at: ${new Date().toISOString()}${colors.reset}`
  );
  console.log("=".repeat(60) + "\n");
};
...
if (require.main === module) {
  ...
  if (typeof window !== "undefined") {
    console.warn(
      "⚠️ This scanner is designed for Node.js terminal execution only. Use the React component for browser scanning."
    );
  } else {
    const fs = require("fs");
    ...
    try {
      console.log(
        `${colors.blue}${colors.bright}🔍 Initializing Shai-Hulud Scanner...${colors.reset}`
      );
      ...
      console.log(
        `${colors.green}✅ Successfully loaded package-lock.json${colors.reset}`
      );
      console.log(
        `${colors.blue}🔍 Scanning for infected packages...${colors.reset}`
      );

      const report = scanForShaiHulud(parsedContent);
      printStyledReport(report);
    } catch (error) {
      console.error(
        `${colors.red}${colors.bright}❌ Error: ${error.message}${colors.reset}`
      );
      console.error(
        `${colors.red}Make sure package-lock.json exists in the current directory${colors.reset}`
      );
    }
  }
}
